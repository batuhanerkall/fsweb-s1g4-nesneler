...
//let menuElemanlari2 = {};
//menuElemanlari2.isim = gelenIsim; //dot notation
//menuElemanlari2["Fiyat"] = gelenFiyat; // bracket notation, sonradan değişiklik yapılaiblir.
...
console.log(MenuElemaniOlustur("serpme kahvalti", 16, "kahvalti"));
...
*/
const yeniElemanOlustur = MenuElemaniOlustur("Karışık Pizza", 5, "Pizzalar");
console.log(yeniElemanOlustur);
/* Görev 2: 
...
*/
let Reynaninyorumu = degerlendirmeler.find((item) => {
...
function AnahtardanDegerlendirmeAl(dizi, index) {
  let comment = dizi[index];
  let metin =
    comment.isim +
    " isimli kişi " +
    comment.puan +
    " puan verdi ve şunları yazdı: " +
    comment.geribildirim;
  return metin;
}
...
function SonDegerlendirmeyiAl(dizi) {
  let comment = dizi[dizi.length - 1];
  let metin =
    comment.isim +
    " isimli kişi " +
    comment.puan +
